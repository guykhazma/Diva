...
//  debug_assert(alen <= key2->klen);
  ...
  struct kv * const anchor = wormhole_alloc_akey(alen);
  if (anchor) {
    u8 buf[key2->key_size + sizeof(key2->payload_ptr)];
    memcpy(buf, &key2->key, key2->key_size);
    memcpy(buf + key2->key_size, key2->payload_ptr, sizeof(key2->payload_ptr));
...
wormhole_int_get(struct wormref_int * const ref, const struct kref * const key, struct kv * const out)
{
  ...
  out->vlen = sizeof(res->payload_ptr);
  memcpy(out->kv + out->klen, res->payload_ptr, out->vlen);
...
// all locks will be released before returning
  static bool
wormhole_split_meta(struct wormref_int * const ref, struct wormleaf_int * const leaf2, void **locked_leaf_addrs)
{
  ...
  if (leaf2->next && locked_leaf_addrs[0] != leaf2->next)
    wormleaf_int_lock_write(leaf2->next, ref);
    ...
    struct kv * const new, void **locked_leaf_addrs)
{
  ...
  wormleaf_int_lock_write(leaf2, ref);
...
wormhole_meta_merge(struct wormref_int * const ref, struct wormleaf_int * const leaf1,
    struct wormleaf_int * const leaf2, void **locked_leaf_addrs)
{
...
// all locks will be released when this function returns
  static bool
wormhole_meta_leaf_merge(struct wormref_int * const ref, struct wormleaf_int * const leaf, void **locked_leaf_addrs)
{
  struct wormleaf_int * const next = leaf->next;
  debug_assert(next);
...
wormhole_int_put(struct wormref_int * const ref, struct kv * const kv, void **locked_leaf_addrs)
{
  ...
  struct wormleaf_int * const leaf = locked_leaf_addrs[0] != NULL ? wormhole_jump_leaf_write_has_lock(ref, &kref)
  ...
  const bool rsi = wormhole_split_insert(ref, leaf, kv, locked_leaf_addrs);
  return rsi;
}
...
{
  wormhole_int_resume(ref);
  const bool r = wormhole_int_put(ref, kv, locked_leaf_addrs);
  wormhole_int_park(ref);
  return r;
}
...
// del {{{
  static void
wormhole_del_try_merge(struct wormref_int * const ref, struct wormleaf_int * const leaf, void **locked_leaf_addrs)
{
  struct wormleaf_int * const next = leaf->next;
...
{
  wormhole_int_resume(ref);
  const bool r = wormhole_int_del(ref, key, locked_leaf_addrs);
  wormhole_int_park(ref);
  return r;
}
...
wormhole_int_iter_skip1(struct wormhole_int_iter * const iter, bool write, bool unlock)
{
  ...
  void
wormhole_int_iter_skip(struct wormhole_int_iter * const iter, const u32 nr, bool write, bool unlock)
{
  u32 todo = nr;
...
wormhole_int_iter_inp(struct wormhole_int_iter * const iter, kv_inp_func uf, void * const priv)
{
  ...
  res->vlen = sizeof(struct store_sim_hack);
  memcpy(res->kv + kv->key_size, kv->payload_ptr, res->vlen);
...
wh_int_create(void)
{
  ...
  return wormhole_int_create(&kvmap_mm_dup);
}
    ...
    const void * const vbuf, const u32 vlen, void **locked_leaf_addrs)
{
  ...
  bool res = wh_api->put(ref, newkv, locked_leaf_addrs);
  free(newkv);
  return res;
}
    ...
    void ** vbuf_out, u32 * const vlen_out)
{
    ...
    *vbuf_out = iter->leaf->kvs[iter->is].payload_ptr;
    *vlen_out = sizeof(iter->leaf->kvs[iter->is].payload_ptr);
}
...
wh_int_iter_inp(struct wormhole_int_iter * const iter, kv_inp_func uf, void * const priv)
{
  ...
  void
wh_int_iter_park(struct wormhole_int_iter * const iter, bool write)
{
  wh_api->iter_park(iter, write);
}

  void
wh_int_iter_destroy(struct wormhole_int_iter * const iter, bool write)
{
  wh_api->iter_destroy(iter, write);
}
// }}} wh
